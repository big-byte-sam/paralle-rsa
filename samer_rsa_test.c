...
int main(int argc, char *argv[])
{
    ...
    int chosen_bit_lengths[4] = {10,12,14,16};
    int chosen_proc[2] = {4,6};

    int _file = 0;

    //for (int _file = 0; _file < 3; _file++){

        for (int _bit = 0; _bit < 4; _bit++ ){

            // running sequential
            sequential(1, chosen_bit_lengths[_bit]);


            for ( int _proc = 0; _proc < 2; _proc++ ){
                // run multithreading
                multi_threads(1, chosen_bit_lengths[_bit], chosen_proc[_proc]);

                // running MPI
                // --- Call the compiled mpi file
                char command[300];
                // --- Embedding variables in a string using sprintf
                sprintf(command, "mpiexec -n %d samer_rsa_mpi.exe %d %d %llu %llu %llu", chosen_proc[_proc], _file+1, chosen_bit_lengths[_bit], public_key, private_key, n);
                printf("========================================================\n");
                printf("RSA using MPI: %d processes, %d bit length, on %s file, PbKey: %llu, PrKey: %llu, N: %llu \n", chosen_proc[_proc], chosen_bit_lengths[_bit], files[_file], public_key, private_key, n);
                printf("========================================================\n");
                // Run MPI program
                system(command);
